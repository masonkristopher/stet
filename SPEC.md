...
`w` switches the active worktree in place and re-points the tree, diffs, refresh (watcher and poll), and checks at it, with no restart. The switch loads only the changed set up front (the same shape startup seeds), so it re-points the instant the cheap diff commands resolve rather than blocking on a full `git ls-files --stage` over the worktree; the full tracked-file tree fills in on the repoFiles poll, exactly as at startup. The picker lists worktrees from `git worktree list --porcelain -z` (bare entries excluded) and marks prunable ones.
