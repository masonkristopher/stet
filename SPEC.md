...
Every overlay (the go-to-file palette, content search, the scope and worktree pickers, the keybindings reference, and the theme switcher) reads as one command-palette family: a centered bordered box at the shared `paletteLeft`/`paletteWidth` and `zIndex 100`, a list, and a muted instruction-hint footer at the bottom (`↑↓ navigate · ⏎ <verb> · esc close`, tailored per overlay and naming only the keys that overlay actually honors). Input-driven overlays (palette, content search, theme switcher) are title-less, the filter placeholder is their label; fixed menus and the keybindings reference carry a short action-verb title (`switch scope`, `switch worktree`, `keyboard shortcuts`). `esc` is the canonical close on all of them. Content search keeps its status-summary row (match counts and the current scope) separate from the instruction hint, so the two never duplicate a keybinding; with no matches the list band is gone and that status row carries the empty hint instead of a blank list. The chrome is deliberately inlined per overlay for now (a shared overlay-shell component is a future extraction), so each picker owns its own rows, swatches, and hover-preview.
