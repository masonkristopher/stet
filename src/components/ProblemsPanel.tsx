...
export function ProblemsPanel() {
  ...
  // The border (2) eats the panel width before the row's own padding.
  const contentWidth = () => Math.max(0, state.terminalWidth() - 2);
            ...
            {(row) => (
              ...
              >
                ...
                <Show when={asFailure(row().item)}>
                  {(item) => (
                    <box width="100%" flexDirection="row" paddingLeft={1 + INDENT} paddingRight={1}>
