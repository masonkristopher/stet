...
export const DiagnosticsLive = Layer.effect(
  ...
  Effect.gen(function* diagnosticsLive() {
    ...
    function runCommand(repoRoot: string, command: CheckerCommand & { command: string[] }) {
      return process.run(command.command, command.cwd ?? repoRoot, { allowedExitCodes: command.allowedExitCodes }).pipe(
        Effect.flatMap((result) => Effect.try({ catch: (error) => error, try: () => command.parser(result) })),
        Effect.map((diagnostics) => ({
          diagnostics: diagnostics.map((diagnostic) => ({
            ...diagnostic,
            path: command.cwd !== undefined && !isAbsolute(diagnostic.path) ? join(command.cwd, diagnostic.path) : diagnostic.path,
          })),
          kind: "ok" as const,
        })),
        Effect.catch((error) =>
          Effect.succeed({ kind: "failed" as const, message: error instanceof Error ? error.message : String(error) }),
        ),
      )
    }

    function runChecker(repoRoot: string, files: ChangedFile[], checker: CheckerName, commands: CheckerCommand[]) {
      const [first] = commands
      if (commands.length === 1 && first?.command === undefined) {
        const message = first?.unavailableMessage ?? `${checker} is not configured`
        return Effect.succeed<CheckerUpdate>({ checker, state: stateForEveryFile(files, "unavailable", message) })
      }
      ...
      return Effect.all(
        ...
        Effect.map((results): CheckerUpdate => {
          ...
          return { checker, state: stateForResolvedChecker(checker, files, allDiagnostics, repoRoot) }
        }),
      )
    }
    ...
    return {
      ...
      run: (repoRoot, files) =>
        Stream.fromIterable([...groupByChecker(discoverCheckerCommands(repoRoot, files)).entries()]).pipe(
          Stream.mapEffect(([checker, commands]) => runChecker(repoRoot, files, checker, commands), { concurrency: "unbounded" }),
        ),
    }
  }),
)
